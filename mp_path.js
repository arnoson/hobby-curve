...
var mp_make_choices = function (knots) {
  ...
  while (true) {
    ...
    if (
      p.rtype > mp_explicit &&
      Math.pow(p.x_pt - q.x_pt, 2) + Math.pow(p.y_pt - q.y_pt, 2) <
        Math.pow(epsilon, 2)
    ) {
      p.rtype = mp_explicit
      if (p.ltype == mp_open) {
    ...
    if (p.rtype >= mp_given) {
      ...
      while (true) {
        ...
        if (k > 0) {
          ...
          psi.push(
            mp_n_arg(
              mp_take_fraction(delta_x[k], cosine) +
                mp_take_fraction(delta_y[k], sine),
              mp_take_fraction(delta_y[k], cosine) -
                mp_take_fraction(delta_x[k], sine)
            )
          )
        }
        k += 1
      ...
      if (q.ltype == mp_open) {
        ...
        if (
          Math.pow(delx_pt, 2) + Math.pow(dely_pt, 2) <
          Math.pow(epsilon, 2)
        ) {
          q.ltype = mp_curl
    ...
    if (k == 0) {
      ...
      if (s.ltype == mp_end_cycle || s.ltype == mp_open) {
        ...
        if (s.ltype == mp_curl) {
          ...
          theta[n] = -mp_make_fraction(
            mp_take_fraction(vv[n - 1], ff),
...
var mp_set_controls = function (p, q, delta_x, delta_y, st, ct, sf, cf) {
  ...
  p.rx_pt =
    p.x_pt +
    mp_take_fraction(
      mp_take_fraction(delta_x, ct) - mp_take_fraction(delta_y, st),
      rr
    )
  p.ry_pt =
    p.y_pt +
    mp_take_fraction(
      mp_take_fraction(delta_y, ct) + mp_take_fraction(delta_x, st),
      rr
    )
  q.lx_pt =
    q.x_pt -
    mp_take_fraction(
      mp_take_fraction(delta_x, cf) + mp_take_fraction(delta_y, sf),
      ss
    )
  q.ly_pt =
    q.y_pt -
    mp_take_fraction(
      mp_take_fraction(delta_y, cf) - mp_take_fraction(delta_x, sf),
      ss
    )
  p.rtype = mp_explicit
  q.ltype = mp_explicit
  ...
  for (var i = 0; i < p.length; i++) {
    knots.push({
      ...
      left_tension: function () {
        if (!this.ly_pt) this.ly_pt = 1
        return this.ly_pt
      },
      right_tension: function () {
        if (!this.ry_pt) this.ry_pt = 1
        return this.ry_pt
      },
      left_curl: function () {
        return this.lx_pt || 0
