const calcDeltaValues = (knots, cyclic) => {
  var _a;
  const end = cyclic ? knots.length : knots.length - 1;
  for (let i = 0; i < end; i++) {
    const knot = knots[i];
    const nextKnot = (_a = knots[i + 1]) != null ? _a : knots[0];
...
const calcPsiValues = (knots, cyclic) => {
...
const calcPhiValues = (knots, cyclic) => {
...
const curlRatio = function(gamma, tensionA, tensionB) {
  const alpha = 1 / tensionA;
  const beta = 1 / tensionB;
  return Math.min(4, ((3 - alpha) * alpha ** 2 * gamma + beta ** 3) / (alpha ** 3 * gamma + (3 - beta) * beta ** 2));
};
const velocity = (thetaSin, thetaCos, phiSin, phiCos, t) => Math.min(4, (2 + Math.sqrt(2) * (thetaSin - phiSin / 16) * (phiSin - thetaSin / 16) * (thetaCos - phiCos)) / (1.5 * t * (2 + (Math.sqrt(5) - 1) * thetaCos + (3 - Math.sqrt(5)) * phiCos)));

const calcThetaValues = function(knots, cyclic) {
  var _a;
  const uu = [];
  const ww = [];
  const vv = [];
  ...
  for (let i = 1; i < passes; i++) {
    const knot = (_a = knots[i]) != null ? _a : firstKnot;
...
const setControlsLine = (knotA, knotB) => {
...
const createHobbyKnots = (points, tension = 1, cyclic = false) => {
  var _a, _b;
  const knots = points.map(({ x, y }) => createKnot(x, y, tension));
  const firstKnot = knots[0];
  const lastKnot = knots[knots.length - 1];
  for (let i = 0; i < knots.length; i++) {
    knots[i].next = (_a = knots[i + 1]) != null ? _a : firstKnot;
    knots[i].prev = (_b = knots[i - 1]) != null ? _b : lastKnot;
  }
...
const createHobbyBezier = (points, { tension = 1, cyclic = false } = {}) => {
...
const createHobbyCurve = (points, { tension = 1, cyclic = false } = {}) => {
  const bezier = createHobbyBezier(points, { tension, cyclic });
  const toString = (point) => [point.x, point.y].join(",");
  const bezierCommands = bezier.map(({ startControl, endControl, point }) => [toString(startControl), toString(endControl), toString(point)].join(" "));
